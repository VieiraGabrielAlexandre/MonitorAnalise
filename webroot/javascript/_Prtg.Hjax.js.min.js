...
stateData.container;if(!!edata&&edata.url){dataurl=edata.url.split("#");dataurl=dataurl[0]+"?"+url.split("?")[1]+(dataurl.length<=1?"&":($container=$("#"+dataurl[1]),"#"+dataurl[1]))}else{dataurl=url;_Prtg.Plugins["crumblerSensorStats"].plugin.id=_Prtg.Util.getUrlParameters(dataurl)["id"]||null}_Prtg.Tip.killPopups();if($container.find(".loadspinner").length==0)$container.append('<div class="loadspinner"></div>');$container.addClass("loading");$(window).scrollTop(0);return $.ajax({url:dataurl,dataType:"html",
data:data,beforeSend:function(jqXHR){jqXHR.peNumber="PE1113"},success:function(data,textStatus,jqXHR){var $data=$("<div/>");$container.removeClass("loading").removeData().children().remove();if(!!edata&&edata.url)$data.html($(data).html());else $data.html(data);timeout=null;var CurrentTitle=window.title=document.title;window.title=document.title=$data.find("[data-title]").attr("data-title")+" | "+sitename;_Prtg.Events.publish("refresh.events.prtg");$container.empty().append($data);_Prtg.SetupPage($container);
!!window.__ga&&window.__ga("send","pageview",window.__gaStripOrigin(dataurl));_Prtg.Events.publish("loaded.events.prtg",$container,stateData.data)}})}function linkClickHandler(e){var $me=$(this);var data=!!$me.data()&&!!$me.data().url&&{url:$me.data().url}||{};if(e.ctrlKey||e.metaKey)return;if($me.hasClass("logout")||$me.attr("href")===undefined||$me.attr("href")==="/home")return;if(window.winGUI===true){window.location.href=e.currentTarget.href;return}if(!!timeout)return;timeout=e;e.preventDefault();
if(!e.data)throw new Error("Hjax.linkHandler: event.data missing namespace and container");else navigate({url:$me.attr("href"),container:e.data.container,namespace:e.data.namespace,data:data});return false}$.extend(true,window,{_Prtg:{Hjax:{loadLink:function(url,namespace,container,data){return navigate.call(null,{url:url||"",namespace:namespace||"linkloader.prtg",container:container||"#container",data:data||null})},bind:bind,unbind:unbind,navigate:navigate}}});$(document).on("click.hjax.prtg",'#header_menu a:not(.nohjax,[href*=":"]), #crumblersensorstats a:not(.nohjax,[href*=":"]), #main a:not([href^="#"], [href*=":"], [target], [onclick], .nohjax, .as-dialog), #header_breadcrumbs a:not(.nohjax), #mainstuff a:not([href^="#"], [href*=":"], [target], [onclick], .nohjax, .as-dialog)',
