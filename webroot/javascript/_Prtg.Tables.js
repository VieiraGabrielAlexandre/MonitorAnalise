...
(function ($, window, document, undefined) {
    ...
    prtgTable.prototype.initEvents = function () {
        ...
        if (self.hasCheckboxes) {
            this.$el.on('click', 'tbody.ui-selectable tr', function (event) {
                if ($(event.target).is('input, .favstar, .favempty, a') || $(this).is('.notselectable') || $(event.target).is('.favstar, .favempty')) return;
                var $this = $(this);
        ...
        this.$el.on('click tab', '.tablemenu a', {self: this}, self.handleTableMenu)
            .on('change', '.selectallasmultiedit', {self: this}, self.toggleSelectAll)
            .on('change', '.tablemultiselectcheckbox', {self: this}, self.selectColumn)
            .on('click', 'td.col-properties a', {self: this}, self.moreInfo)
            .on('click', '.tablebuttonbox .actionbutton', {self: this}, self.tablebuttonclickhandler)
            .on('change changed', 'div.sensorlookupnew,.select-reloadtablelink:not(.disabled),#tagfilter', $.proxy(self.reloadTableLink, this))
        ...
        this.$parent
            .on('click', '.reloadtablelink:not(.disabled)', $.proxy(self.reloadTableLink, this));
    ...
    prtgTable.prototype.handleTableMenu = function (e) {
        ...
        if (action === 'stop') {
            ...
            _Prtg.objectTools.simulateObject(ids.join(','), 1);
        } else if (action === 'prio') {
    ...
    prtgTable.prototype.updateTableMenuPosition = function () {
        var container = $('.js-fixedtablemenu');
        var shouldReposition = !$('body').hasClass('device-body');
        container.attr('style','');
        if (container && shouldReposition && container.length === 1) { // only add fixed menu if there is only one table on the page
            var tableRightPos = $(document).outerWidth() -  (container.offset().left + container.outerWidth());
            container.css({
                'position': 'fixed',
                'top': container.offset().top,
