$.validator.addMethod("dateDE",function(value,element){return this.optional(element)||/^\d\d?\.\d\d?\.\d\d\d?\d?$/.test(value)},'<#langjs key="js.validate.date" default="Please enter a valid date.">');$.validator.addMethod("numberDE",function(value,element){return this.optional(element)||/^-?\d*\,?\d*$/.test(value)},'<#langjs key="js.validate.number" default="Please enter a valid number.">');
$.validator.addMethod("number",function(value,element){return this.optional(element)||/^-?\d*\.?\d*$/.test(value)},'<#langjs key="js.validate.number" default="Please enter a valid number.">');$.validator.addMethod("selectrequired",function(value,element){return this.optional(element)||(value=""||!/^(\|+)$/.test(value))},'<#langjs key="js.validate.select" default="Please select a value.">');
...
helpText=helpText.replace("PURGEDATE",_Prtg.Options.lastpurgedate);return helpText});$.validator.addMethod("requiredtablecheckbox",function(value,element){var d=$(element).closest("table.dataTable");if(d.length==0)return true;d=d.DataTable();return $("input.checkbox",d.rows().nodes()).toArray().some(function(e){return e.checked})},'<#langjs key="js.validate.select" default="Please select a value.">');
$.validator.addMethod("ticketnumber",function(value,element){var number=value.substr(3);return this.optional(element)||number>1E5&&/^(pae|PAE)[0-9]{6}[0-9]?$/.test(value)},'<#langjs key="js.validate.ticketnumber" default="Please enter your ticket number in the form PAE plus seven digits">');$.validator.addMethod("showrequired",function(value,element){return false},'<#langjs key="js.validate.select" default="Please select a value.">');
...
$.validator.addMethod("oneof",function(value,element){var selected=$(element).data("plugin_prtgDatetimepicker").getCurrentSelectedDate();var arr=_Prtg.Options.lastpurgedate.split("-");var lastpurge=new Date(arr[0],arr[1]-1,arr[2],arr[3],arr[4],arr[5]);var result=false;if(selected.getTime()>lastpurge.getTime())result=true;if($(element).hasClass("orginal_input"))result=true;return this.optional(element)||result},function(){var helpText='<#langjs key="js.validate.purgedate" default="Warning: You cannot generate the historic data report because data is only available as of the last data purge on PURGEDATE. Please adjust the start date accordingly.">';
helpText=helpText.replace("PURGEDATE",_Prtg.Options.lastpurgedate);return helpText});
$.validator.addMethod("validgroup",function(value,element,options){var $fields=$("[data-rule-validgroup='"+options+"']:visible",element.form),$fieldsFirst=$fields.eq(0),validator=$fieldsFirst.data("valid_req_grp")?$fieldsFirst.data("valid_req_grp"):$.extend({},this),isValid=$fields.filter(function(){return validator.elementValue(this)}).length>=1;$fieldsFirst.data("valid_req_grp",validator);if(!$(element).data("being_validated")){$fields.data("being_validated",true);$fields.each(function(){validator.element(this)});
$fields.data("being_validated",false)}return isValid},$.validator.format('<#langjs key="js.validate.pleaseenteralimit" default="Enter a value.">'));
